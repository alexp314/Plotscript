...
public:
 ...
 // void addExpLam(const Atom & sym, const Expression & exp);
  ...
  */
  ...
  //std::map<std::string, EnvResult> propertyMap;
...
private:
  ...
  std::map<std::string, EnvResult> localMap;
...
Expression applyL(const std::vector<Expression>& args);
